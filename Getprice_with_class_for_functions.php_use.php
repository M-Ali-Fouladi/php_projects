...
if (!isset($response->errors)) {
		...
		unset($body);
			$body_preg = preg_replace(
				...
				"",
				""), $body_iconv);
			unset($body_iconv);//here we start------------------------------------
			$doc = new DOMDocument;
			$body_preg = mb_convert_encoding($body_preg, 'HTML-ENTITIES', 'UTF-8');
			@$doc->loadHTML('<?xml encoding="utf-8" ?>' . $body_preg);
			$xpath = new DOMXPath($doc);
$query = '//html/body/main/div[2]/div[1]/div/article/section[1]/div[2]/div[2]/div/div[1]/div[1]/div[9]/div[2]/div';//--------xpath----------------mohammadadali updated
